...
def fit_one_epoch(model_train, model, loss, optimizer, epoch, epoch_step, epoch_step_val, gen, genval, Epoch, cuda):
    ...
    with tqdm(total=epoch_step, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            output = loss(outputs, targets.long())

            output.backward()
            optimizer.step()

            with torch.no_grad():
                equal = torch.eq(torch.argmax(outputs, dim=1), targets)
    ...
    with tqdm(total=epoch_step_val, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        for iteration, batch in enumerate(genval):
            ...
            with torch.no_grad():
                if cuda:
                    images_val = images_val.type(torch.FloatTensor).cuda()
                    targets_val = targets_val.type(torch.FloatTensor).cuda()
                else:
                ...
                optimizer.zero_grad()
                outputs = model_train(images_val)
                output = loss(outputs, targets_val.long())

                equal = torch.eq(torch.argmax(outputs, dim=1), targets_val)
                accuracy = torch.mean(equal.float())
